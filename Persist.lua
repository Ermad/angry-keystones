...
function ScenarioTimer_OnUpdate(self, elapsed)
	if not Addon.Config.persistTracker then return ScenarioTimer_OnUpdate_Old(self, elapsed) end
...
function ScenarioTimer_Start(block, updateFunc)
	if not Addon.Config.persistTracker then return ScenarioTimer_Start_Old(block, updateFunc) end
...
function ScenarioTimer_Stop(...)
	if not Addon.Config.persistTracker then return ScenarioTimer_Stop_Old(...) end
...
function SCENARIO_CONTENT_TRACKER_MODULE:StaticReanchor()
	if not Addon.Config.persistTracker then return SCENARIO_CONTENT_TRACKER_MODULE_StaticReanchor_Old(self) end
...
function SCENARIO_CONTENT_TRACKER_MODULE:Update()
	if not Addon.Config.persistTracker then return SCENARIO_CONTENT_TRACKER_MODULE_Update_Old(self) end
	local inCompletedInstance = IsInCompletedInstance()
...
function SCENARIO_CONTENT_TRACKER_MODULE:UpdateCriteria(numCriteria, objectiveBlock)
	if not Addon.Config.persistTracker then return SCENARIO_CONTENT_TRACKER_MODULE_UpdateCriteria_Old(self, numCriteria, objectiveBlock) end
	if IsInCompletedInstance() then
